...
<body>
	<div class="wrapper cf">
		...
		<div class="toggle-container">
			<?php
				$num_soc1 = $_GET['num_soc1'];
				$num_soc2 = $_GET['num_soc2'];
				$tipo_busq = $_GET['tipo_busq'];
				if(isset($num_soc1) and isset($num_soc2) and isset($tipo_busq)){
					
					$consulta = "SELECT sp_baja_familiar_socio($tipo_busq,null,$num_soc1,$tipo_busq,null,$num_soc2)";
					if (!$result = pg_query($connect,$consulta)) {
						echo '<p class="infobox-error">'.pg_last_error($connect).'</p><br>';
					} else {
						echo '<p class="infobox-success">Se eliminó con éxito</p><br>';
					}
